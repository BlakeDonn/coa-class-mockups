...
| Door role art | Round 8 (user ask): WoW icon art replaces the text glyphs — ⚔ `ability_dualwield` · 🛡 `ability_defend` · ✚ `spell_chargepositive` · ⚑ `inv_banner_02` (the WoW war banner as the Support flag). The literal LFG atlas is not publicly hosted; client extraction is the recorded follow-up if exact art is wanted | PROPOSED (awaiting look) |
| Door middle element | Round 8 variants for "one more thing" between name and range: **M1** family name with tagline tooltip (covers all 70, zero authoring) · **M2** authored 3–4 words (Cultist/Tinker drafts only) · **M3** the ruled verb chip (Cultist/Tinker only) | OPEN (awaiting pick) |
