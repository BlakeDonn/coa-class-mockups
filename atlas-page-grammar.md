...
| Door role art | Round 8, HD fix: the client's own role texture is only 16px (user: "really low quality"). Final assets are **64px frames of WoW's UI-LFG-ICON-ROLES** via the canonical interface-art mirror (Gethe/wow-ui-textures), stored in `generated-assets/lfg-*.png`. Damage sword · Tank shield · Healer cross · Support = the texture's **green flag** (the user's original instinct, now in matching gold-rim style; supersedes the crown pick made when flag meant a mismatched banner tile). The 16px crown stays the recorded alternative | PROPOSED (flag-vs-crown confirm) |
| Door middle element | Round 8 variants: **M1** family name + tagline tooltip (all 70, zero authoring) · **M2** archetype tag, "2H Void bruiser" style — EXTRACTED from the researched fantasy sentence, never invented; 11 specs drafted (Cultist, Tinker, Witch Hunter), the ×70 pass is compression over existing text · **M3** the ruled verb chip (Cultist/Tinker only) | OPEN (awaiting pick) |
