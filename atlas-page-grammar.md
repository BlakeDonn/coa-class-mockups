...
| Door role art | Round 8, corrected to the user's actual ask: **the real queue icons, extracted from the Ascension client itself** (`patch-A.MPQ` → `Interface\lfgframe\lfgrole.blp`, native 16px, stored in `generated-assets/lfg-*.png`). Damage sword · Tank shield · Healer cross. Support has no queue icon: the texture's leader crown is the default, the WoW banner tile the recorded alternative — user to pick | PROPOSED (awaiting look + crown/banner pick) |
